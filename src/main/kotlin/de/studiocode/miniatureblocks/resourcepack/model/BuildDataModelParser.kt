...
class BuildDataModelParser(buildData: BuildData) {
    ...
    private fun registerBlockTextures(usedMaterials: ArrayList<Material>) {
        val blockTextures =
            usedMaterials.filter { BlockTexture.hasMaterial(it) }.map { BlockTexture.getFromMaterial(it) }
        var id = 0
    ...
    private fun addVoxelTextures(element: JsonObject, buildBlockData: BuildBlockData, cube: Cube) {
        ...
        for ((side, texture) in cube.textures) {
            if (!buildBlockData.hasBlock(side)) { // don't add a texture when it can't be seen
